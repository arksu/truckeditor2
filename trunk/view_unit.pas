...
type
  ...
  Tview_form = class(TForm)
    ...
    procedure view_panelDblClick(Sender: TObject);
...
procedure Tview_form.FormCreate(Sender: TObject);
begin
  max := false;
end;

procedure Tview_form.FormMouseWheelDown(Sender: TObject; Shift: TShiftState;
  MousePos: TPoint; var Handled: Boolean);
begin
  vp.Camera.Scale := vp.Camera.Scale - MOUSE_WHEEL_K * SCALE_K;
  if vp.Camera.Scale < 0 then vp.Camera.Scale := 0;
end;

procedure Tview_form.FormMouseWheelUp(Sender: TObject; Shift: TShiftState;
  MousePos: TPoint; var Handled: Boolean);
begin
  vp.Camera.Scale := vp.Camera.Scale + MOUSE_WHEEL_K * SCALE_K;
  if vp.Camera.Scale < 0 then vp.Camera.Scale := 0;
...
procedure Tview_form.view_panelDblClick(Sender: TObject);
begin
  if not max then
  begin
    old_rect := rect(left, top, width, height);
    left := 0;
    top := 0;
    ...
    begin
      left := old_rect.Left;
      top := old_rect.Top;
      width := old_rect.Right;
      height := old_rect.Bottom;
    end;
  max := not max;
end;
