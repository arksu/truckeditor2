...
type
  ...
  public
    ...
    function  first_selected : integer;
    procedure move_selected(dx,dy,dz: extended);
    function  to_grid(value : extended) : extended;

    function get_free_nodeid : integer;
    procedure clone_selected;
    function  add_new_node(ax, ay, az : extended) : integer;
...
const
  ...
  NODE_RANGE = 5; // размер квадрата вокруг ноды для выбора в виде
...
function Teditor_form.add_new_node(ax, ay, az : extended): integer;
begin
...
procedure Teditor_form.clear_selected;
var
  i : integer;
begin
...
procedure Teditor_form.deselect_all;
var
  i : integer;
begin
  for i := 0 to num_nodes(truck) - 1 do
...
function Teditor_form.first_selected: integer;
var
  i : integer;
begin
  result := -1;
  ...
  for i := 0 to Length(truck.nodes) - 1 do
    with truck.nodes[i] do begin
        ...
        // HIGHLIGHT
          ...
          glColor3f(point_color[1], point_color[2], point_color[3]);
        end;
        glBegin(GL_POINTS);
...
begin
  ...
  if (button=mbleft) and (mode_nodes.Down) and (nodes_select.Down) then begin
    ...
    end_rect := point(X, Y);
    if not (ssCtrl in Shift) then clear_selected;
  ...
  // NODES MOVE
  if (button=mbleft) and (mode_nodes.Down) and (nodes_move.Down) and (first_selected >= 0) then begin
    old_point.x := truck.nodes[first_selected].x;
    old_point.y := truck.nodes[first_selected].y;
    old_point.z := truck.nodes[first_selected].z;
...
begin
  ...
  // NODES MOVE
  if (ssLeft in Shift) and (mode_nodes.Down) and (nodes_move.Down) and (first_selected >= 0) then begin
...
procedure Teditor_form.move_selected(dx, dy, dz: extended);
var
  i : integer;
begin
  for i := 0 to num_nodes(truck) - 1 do
...
begin
  ...
  if (button=mbleft) and (mode_nodes.Down) and (nodes_add.Down) then begin
    add_new_node(cur_add_node.x, cur_add_node.y, cur_add_node.z);
  end;
end;
...
begin
  ...
  set_color(F.ReadString('main','point_color',  'FFFFFF'), point_color);
...
begin
  ...
  if (not mode_nodes.Down) or (not nodes_select.Down) then exit;  
  ...
  begin
    ...
    for i := 0 to num_nodes(truck) - 1 do begin
      ...
      if  (wx > cpos.X - NODE_RANGE)
      and (wx < cpos.X + NODE_RANGE)
      and (wy > cpos.Y - NODE_RANGE)
      and (wy < cpos.Y + NODE_RANGE) then begin
