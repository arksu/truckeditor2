...
type
  ...
  private
    ...
    Distance  : Single;
    MDrag_main, Mdrag_left, mdrag_front, mdrag_top : tmy_drag;
    MPos_main, mpos_left, mpos_front, mpos_top  : TPoint;

    old_panels_x, old_panels_y : integer;
    main_downed : boolean;
    // радиус точки
    points_radius : single;
    // цвет фона в проекциях
    ...
    panel_rect : tpanel;
    opened_file : string;
    old_point, grid_point, cur_add_node : tmy_vector;
...
procedure Teditor_form.add_beam(node_id1, node_id2: integer);
var
  new_id : integer;
begin
  new_id := num_beams(truck);
  setlength(truck.beams, num_beams(truck)+1);
  truck.beams[new_id].id_begin := node_id1;
  truck.beams[new_id].id_end := node_id2;
  truck.beams[new_id].options := '';
  truck.beams[new_id].highlight := false;
  truck.beams[new_id].selected := Autoselectnewnodes1.Checked;
end;

function Teditor_form.add_node(ax, ay, az : extended): integer;
begin
  setlength(truck.nodes , num_nodes(truck)+1);
  truck.nodes[num_nodes(truck)-1].id := get_free_nodeid;
  truck.nodes[num_nodes(truck)-1].x := ax;
  truck.nodes[num_nodes(truck)-1].y := ay;
  truck.nodes[num_nodes(truck)-1].z := az;

  truck.nodes[num_nodes(truck)-1].options := '';

  truck.nodes[num_nodes(truck)-1].selected := Autoselectnewnodes1.Checked;
  truck.nodes[num_nodes(truck)-1].highlight := false;
  result := num_nodes(truck)-1;
end;
...
function Teditor_form.beam_exist(node_id1, node_id2: integer): boolean;
var
  i : integer;
begin
  result := false;
  for i := 0 to num_beams(truck) - 1 do
    if ((truck.beams[i].id_begin = node_id1) and (truck.beams[i].id_end = node_id2)) or
       ((truck.beams[i].id_begin = node_id2) and (truck.beams[i].id_end = node_id1)) then begin
          result := true;
          exit;
    end;
end;
...
begin
  ...
  truck.name := '';
  setlength(truck.nodes, 0);
  setlength(truck.beams, 0);
  setlength(truck.authors, 0);
  opened_file := '';
end;

procedure Teditor_form.deselect_beams;
var
  i : integer;
begin
  for i := 0 to num_beams(truck) - 1 do
...
procedure Teditor_form.deselect_nodes;
var
  i : integer;
begin
  for i := 0 to num_nodes(truck) - 1 do
    truck.nodes[i].selected := false;
end;

procedure Teditor_form.clone_selected;
var
  i, new_id : integer;
begin
  for i := 0 to num_nodes(truck) - 1 do
    if truck.nodes[i].selected then begin
      truck.nodes[i].selected := false;
      setlength(truck.nodes, num_nodes(truck)+1);
      new_id := num_nodes(truck)-1;
      truck.nodes[new_id].id := get_free_nodeid;;
      truck.nodes[new_id].x := truck.nodes[i].x;
      truck.nodes[new_id].y := truck.nodes[i].y;
      truck.nodes[new_id].z := truck.nodes[i].z;
      truck.nodes[new_id].options := truck.nodes[i].options;
      truck.nodes[new_id].selected := true;
      truck.nodes[new_id].highlight := false;
    end;
end;

procedure Teditor_form.connect_sel_nodes;
  procedure connect_with_node(aid : integer);
  var
    j : integer;
  begin
    for j := aid+1 to num_nodes(truck) - 1 do
      if truck.nodes[j].selected then
        if not beam_exist(truck.nodes[aid].id, truck.nodes[j].id) then
          add_beam(truck.nodes[aid].id, truck.nodes[j].id);
  end;
...
procedure Teditor_form.delete_beam(array_id: integer);
var
  i : integer;
begin
  for i := array_id to num_beams(truck) - 2 do
    truck.beams[i] := truck.beams[i+1];
  setlength(truck.beams, num_beams(truck) -1);
end;

function Teditor_form.delete_beam_bynode(nodeid: integer) : boolean;
var
  i : integer;
begin
  result := false;
  for i := 0 to num_beams(truck) - 1 do
    if (truck.beams[i].id_begin = nodeid) or (truck.beams[i].id_end = nodeid)  then begin
      delete_beam(i);
      result := true;
      exit;
    end;
...
procedure Teditor_form.delete_node(array_id: integer);
var
  i : integer;
  beam_del : boolean;
begin
  repeat
    beam_del := delete_beam_bynode(truck.nodes[array_id].id);
  until not beam_del;
  for i := array_id to num_nodes(truck) - 2 do
    truck.nodes[i] := truck.nodes[i+1];
  setlength(truck.nodes, num_nodes(truck) -1);
end;
...
function Teditor_form.find_nodeid(aid: integer): integer;
var
  i : integer;
begin
  result := -1;
  for i := 0 to num_nodes(truck) - 1 do
    if truck.nodes[i].id = aid then begin
      result := i;
      exit;
    end;
end;

function Teditor_form.first_selected_node: integer;
var
  i : integer;
begin
  result := -1;
...
// NODES
  ...
  // highlight new beam
  if (beams_add.Down) and (mode_beams.Down) and (num_selected_nodes = 1) and (num_hl_nodes = 1) then begin
        glColor3f(beam_hl_color[1],beam_hl_color[2],beam_hl_color[3]);
        glLineWidth(3);
        glBegin(GL_LINES);
        for i := 0 to num_nodes(truck) - 1 do
...
begin
  ...
  if (button=mbleft) and (mode_nodes.Down) and (nodes_select.Down) then begin
    ...
    end_rect := point(X, Y);
    if not (ssCtrl in Shift) then deselect_nodes;
    for i := 0 to num_nodes(truck) - 1 do begin
  ...
  // NODES MOVE
  if (button=mbleft) and (mode_nodes.Down) and (nodes_move.Down) and (first_selected_node >= 0) then begin
    old_point.x := truck.nodes[first_selected_node].x;
    old_point.y := truck.nodes[first_selected_node].y;
    old_point.z := truck.nodes[first_selected_node].z;
  ...
  if (button=mbleft) and (mode_beams.Down) and (beams_select.Down) then begin
    if not (ssCtrl in Shift) then deselect_beams;
    for i := 0 to num_beams(truck) - 1 do begin
  ...
  if (button=mbleft) and (mode_beams.Down) and (beams_add.Down) then begin
    if num_selected_nodes > 2 then begin
      deselect_nodes;
      exit;
    end;
    if num_selected_nodes <=1 then
      for i := 0 to num_nodes(truck) - 1 do begin
      if truck.nodes[i].highlight then begin
...
begin
  ...
  // NODES MOVE
  if (ssLeft in Shift) and (mode_nodes.Down) and (nodes_move.Down) and (first_selected_node >= 0) then begin
...
procedure Teditor_form.move_selected_node(dx, dy, dz: extended);
var
  i : integer;
begin
  for i := 0 to num_nodes(truck) - 1 do
    if truck.nodes[i].selected  then begin
      truck.nodes[i].x := truck.nodes[i].x + dx;
      truck.nodes[i].y := truck.nodes[i].y + dy;
      truck.nodes[i].z := truck.nodes[i].z + dz;
    end;
end;
...
function Teditor_form.num_hl_nodes: integer;
var
  i : integer;
begin
  result := 0;
...
function Teditor_form.num_selected_nodes: integer;
var
  i : integer;
begin
  result := 0;
...
begin
  ...
  if (button=mbleft) and (mode_nodes.Down) and (nodes_add.Down) then begin
    add_node(cur_add_node.x, cur_add_node.y, cur_add_node.z);
  end;
...
function Teditor_form.get_free_nodeid: integer;
begin
  result := 0;
...
procedure Teditor_form.SaveFile(fname: string);
begin
{ TODO :  save file}
end;
...
procedure Teditor_form.set_selected_node(index: integer);
begin
  truck.nodes[index].selected := true;
end;
...
begin
  i := 0;
  while i < num_nodes(truck)  do begin
    if truck.nodes[i].selected then begin
      delete_node(i);
      i := 0;
      continue;
...
begin
  ...
  then begin
    ...
    glGetDoublev (GL_PROJECTION_MATRIX, @ProjMatrix);
    for i := 0 to num_beams(truck) - 1 do begin
      nid_begin := find_nodeid(truck.beams[i].id_begin);
      nid_end := find_nodeid(truck.beams[i].id_end);
